  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8000/getTotalBookingsForItineraryTourGuide/${tourguideId}`);
        if (!response.ok) {
        ...
        console.log(result);
        if (result.report.itineraries && Array.isArray(result.report.itineraries)) {
