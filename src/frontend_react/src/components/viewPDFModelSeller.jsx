...
const ViewPDFModelSeller = () => {
  ...
  const [sellerId, setGuideID] = useState(null);
  ...
  const handleFileUpload = async (event) => {
    ...
    formData.append('ownerId', sellerId);
    ...
    try {
      const response = await fetch('http://localhost:8000/uploadSellerDocuments', {
        method: 'POST',
        body: formData,
      });
      ...
      const refreshResponse = await fetch(`http://localhost:8000/getSellerDocuments/${sellerId}`);
...
export default ViewPDFModelSeller;
