...
const LegendEvents2 = ({ height = 350 }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:8000/getTotalBookings');
        if (!response.ok) {
  ...
  const data = {
    labels: chartData.map(item => item.itemDetails.Name),
    datasets: [
      {
