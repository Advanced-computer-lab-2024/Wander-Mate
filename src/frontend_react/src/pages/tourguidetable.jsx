...
const TourGuideTable = () => {
  ...
  const deletetourguide = async (tourGuide) => {
    const deleteTourguide = axios.delete(
      "http://localhost:8000/deleteaccount",
      {
        data: { Username: tourGuide.Username, userID: tourGuide._id },
      }
    );
    toast.promise(
      deleteTourguide,
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setTourGuides(
          tourGuides.filter(
            (tourGuide1) => tourGuide1.Username !== tourGuide.Username
          )
        );
              ...
              <TableRow>
                ...
                <TableCell className="flex items-center justify-start px-4 py-6">
                  <Button
                    ...
                    onClick={() => deletetourguide(tourGuide)}
                  >
                ...
                <TableRow>
                  <TableCell colSpan={4}>
                    <div className="pl-12 flex flex-col items-start">
                      <p>
                        Created At:{" "}
                        {new Date(tourGuide.createdAt).toLocaleDateString()}
                      </p>
                      <p>
                        Updated At:{" "}
                        {new Date(tourGuide.updatedAt).toLocaleDateString()}
                      </p>
                      {tourGuide.MobileNumber && (
                        <p>Mobile: {tourGuide.MobileNumber}</p>
                      )}
                      {tourGuide.PreviousWork && (
                        <p>Previous Work: {tourGuide.PreviousWork}</p>
                      )}
                      {tourGuide.YearsOfExperience && (
                        <p>Experience: {tourGuide.YearsOfExperience} years</p>
                      )}
                      {tourGuide.averageRating && (
                        <p>Average Rating: {tourGuide.averageRating}</p>
                      )}
                      {tourGuide.totalRatings && (
                        <p>Total Ratings: {tourGuide.totalRatings}</p>
                      )}
                    </div>
                  </TableCell>
                </TableRow>
