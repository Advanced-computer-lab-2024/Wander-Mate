...
const HistoricalTagsPage = () => {
  ...
  const fetchTags = async () => {
    try {
      const response = await axios.get(`${API_URL}/readHistoricalTags`);
      setTags(
        response.data.map((tag) => ({
          ...tag,
          icon:
            iconOptions.find((option) => option.value === tag.icon)?.icon || (
  ...
  const handleCreateTag = async () => {
    if (newTagName.trim() !== '') {
      try {
        const response = await axios.post(`${API_URL}/createHistoricalTags`, {
  ...
  const handleUpdateTag = async (id, newName) => {
    try {
      const response = await axios.patch(`${API_URL}/updateHistoricalTags/${id}`, {
        Name: newName
      });
  ...
  const handleDeleteTag = async (id) => {
    try {
      await axios.delete(`${API_URL}/deleteHistoricalTags/${id}`);
