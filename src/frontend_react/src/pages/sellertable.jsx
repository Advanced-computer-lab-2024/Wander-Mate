...
const SellerTable = () => {
  ...
  const deleteSeller = async (seller) => {
    const deleteseller = axios.delete("http://localhost:8000/deleteaccount", {
      data: { Username: seller.Username, userID: seller._id },
    });
    toast.promise(
      deleteseller,
      {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setSellers(
          sellers.filter((seller1) => seller1.Username !== seller.Username)
        );
  ...
  return (
    <>
      ...
      <Toaster />
      <br></br>
      <h1 className="text-3xl font-bold mb-6 ml-6">All Sellers</h1>
      <Table>
