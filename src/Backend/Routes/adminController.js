...
const viewMyNotificationsAdmin = async (req, res) => {
  const { adminId } = req.params; // Assuming the tourist ID is passed as a URL parameter

  try {
    // Find the notifications for the given tourist ID
    const notifications = await Notification.findOne({ userID: adminId });
...
const removeNotificationAdmin = async (req, res) => {
  const { adminId, notificationId } = req.params; // Get touristId and notificationId from the request body
  try {
    // Validate input
    ...
    const notificationDoc = await Notification.findOne({ userID: adminId });
